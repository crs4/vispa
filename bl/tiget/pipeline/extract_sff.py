...
# Based on http://bioinf.comav.upv.es/sff_extract/ version 0.2.10

# Original version:
#   Copyright (C) 2008 Jose Blanca, COMAV, and Bastien Chevreux
...
# Changes wrt original: feedback is written to a log file instead of stdout
...
def fragment_sequences(sequence, qualities, splitchar):
    ...
    if len(sequence) != len(qualities):
        raise RuntimeError("Internal error: length of sequence (%r) and qualities (%r) don't match" % (sequence, qualities))
...
def extract_reads_from_sff(config, sff_files, logf):
    ...
    #we go through all input files
    for sff_file in sff_files:
        logf.write("Working on '" + sff_file + "':\n")
        ...
        if not debug and config['pelinker_fname']:
            logf.write("Creating temporary sequences from reads in '" + sff_file + "' ... \n")
            ...
            launch_ssaha(config['pelinker_fname'], tmpfasta_fh.name, tmpssaha_fh, logf)
            tmpfasta_fh.close()

            tmpssaha_fh.seek(0)
            read_ssaha_data(tmpssaha_fh, logf)
            tmpssaha_fh.close()
        ...
        logf.write("Converting '" + sff_file + "' ... \n")
        sff_fh = open(sff_file, 'rb')
        #read_header(infile)
        header_data = read_header(fileh=sff_fh)
        ...
        for seq_data in sequences(fileh=sff_fh, header=header_data):
            ...
            #if nseqs_sff >1000:
            #    check_for_dubious_startseq(seqcheckstore,sff_file,seq_data, logf)
            #    sys.exit()
            ...
            if ssahapematches.has_key(seq_data['name']):
                ...
                nseqs_out += 1
        logf.write("done.\n")
        logf.write('Converted %s reads into %s sequences.\n' %
                   (nseqs_sff, nseqs_out))
        sff_fh.close()
...
def check_for_dubious_startseq(seqcheckstore, sffname,seqdata, logf):
...
def tests_for_ssaha(logf):
    '''Tests whether SSAHA2 can be successfully called.'''
    
    try:
        logf.write("Testing whether SSAHA2 is installed and can be launched ... \n")
        fh = open('/dev/null', 'w')
        retcode = subprocess.call(["ssaha2"], stdout = fh)
        fh.close()
        logf.write("ok.\n")
    except :
        logf.write("nope? Uh oh ...\n\n")
...
def launch_ssaha(linker_fname, query_fname, output_fh, logf):
    '''Launches SSAHA2 on the linker and query file, string SSAHA2 output
    into the output filehandle'''

    tests_for_ssaha(logf)

    try:
        logf.write("Searching linker sequences with SSAHA2 (this may take a while) ... \n")
...
def read_ssaha_data(ssahadata_fh, logf):
    ...
    logf.write("Parsing SSAHA2 result file ... \n")
    sys.stdout.flush()
...
def read_config():
    ...
    parser.add_option('--logfile', type=str,
    ...
    if config['want_fastq']:
        ...
        if config['qual_fname'] == def_qual_fname:
            config['qual_fname'] = config['basename'] + '.qual'
...
def debug(logf=sys.stderr):
    try:
        ...
        extract_reads_from_sff(config, args, logf)
...
def main(argv):
    ...
    try:
        ...
        logf.write(str(errval)+"\n")
        return 1
    finally:
        if logf is not None and logf is not sys.stderr:
            logf.close()
